...
- **RESOLVED (2026-08-28) — preset validation exists.** `workbench/tests/presets.spec.ts` checks every preset's shape, that each references only plugins that actually exist in the workspace, that its persona carries the policy-governed sentence and says what to do on a denial, and that capability grants are exclusive — only `research` has web access, only `code-analysis` a code runtime, only `engineering-vision` vision tools, only `artifact` the generators. A preset is plain YAML nothing compiles, so a typo would otherwise survive every gate in the repo and fail at boot. Original note: A flat `.cordis.yml` file cannot be validated against the composed tree without booting the harness. Proposed: a lightweight `verify-wb-presets` script that checks YAML syntax, row shape (`id`/`name` present), tool-name membership against §7.5, and persona policy-governed statement presence. Currently verification is structural assertions only, not a harness dry-run.
...
- **RESOLVED (2026-08-28) — §7.3 `WbPolicyService` gains `governance()` and `setRoleOverride()`.** §6.11 specifies that `wb-admin-console` edits the per-role override table `wb-policy` reads, but the frozen interface exposed only `evaluate()`, so there was no write path and the console could not be built without inventing a second policy store — the one thing §6.11 forbids. `governance()` returns a deep copy for rendering; `setRoleOverride()` validates the role, capability and decision kind and throws on anything outside the frozen unions, so a bad edit fails at the call rather than silently never matching at evaluate() time.

- **RESOLVED (2026-08-28) — §7.2 gains `WbCapability`, `WbPolicyMatrix` and `WbRoleOverrides`.** These described the §5 matrix but lived in `wb-policy`'s own source, so any plugin rendering or editing the matrix would have had to import a sibling's package, which §7 forbids. `wb-policy` now aliases them from `wb-types`, keeping one definition.

- **RESOLVED (2026-08-28) — §7.4 gains `wb/policy/override-changed`, and §7.2's `WbAuditEntry.kind` gains `'policy_override'`.** An admin changing the override table changes what the system will allow; invariant 4 makes that as observable as a decision. `wb-policy` publishes the event and `wb-audit` records it.

- **wb-admin-console (2026-08-28):** §7.3 `WbAuditService` has no change notification and no limit/time-range on `query(filter)`, so the console polls every 4s and re-reads the whole JSONL log each tick, capping the feed at 50 rows client-side. Adequate for the demo; a `subscribe` and a bounded query belong on the audit seam before real volumes. Relatedly, `WbIdentityService.current()` is a per-session lookup with no enumeration, so the console can only report users it has observed acting — there is no way to list the directory.

- **RESOLVED (2026-08-28) — §7.2 `WbPolicyRequest.action` gains `ingest_document`, and §7.3 `WbIngestionService.enqueue` takes a `WbIngestFile`.** This closes the `wb-ingestion` gap logged on 2026-08-28: none of the four original actions described "admit a document to the corpus" (`send_data` implies egress), so the plugin injected `wb-policy` and never called it, leaving every upload ungoverned. `ingest_document` resolves to the `internal_rag` capability — it is local write-side work — and `enqueue` now names the uploading principal and session so the upload is authorized before any content is parsed or indexed. Note the consequence: loading the corpus is itself governed, so a low-clearance principal can no longer seed material above their own clearance.

- **wb-ingestion (2026-08-28):** OOXML text extraction and content-based auto-classification are implemented (`src/office.ts`, `src/classify.ts`). Auto-classification only ever raises a band and never lowers one, so §9 invariant 6 is structural. Remaining scope limit: extraction is text-only — images embedded in a `.docx` are not routed to OCR, and comments, revision marks and speaker notes are not read. A document whose content lives entirely in embedded images will ingest as "no extractable text" and fail loudly rather than entering the corpus empty.
